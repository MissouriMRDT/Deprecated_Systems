...
// Send/Receive bytes to/from slave device
...
int main(void) {
	...
	while (!xflag) {
		...
		case 1: // Select I2C peripheral
			i2cDev = I2C0;
			break;

		case 2: // Toggle mode POLLING/INTERRUPT
			i2c_set_mode(i2cDev, !(mode_poll & (1 << i2cDev)));
			break;

		case 3: // Probe for slave devices
			i2c_probe_slaves(i2cDev);
			break;

		case 4: // Read slave data
			i2c_rw_input(&xfer, 1);
