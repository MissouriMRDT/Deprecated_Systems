...
void LSM9DS1::calcGyro() {
  ...
  /*gx = dof.convGyro(dof.gx);
  gy = dof.convGyro(dof.gy);
  gz = dof.convGyro(dof.gz);*/
  if(dof.gx > -deadzone_raw && dof.gx < deadzone_raw)
    dof.gx = 0;
  if(dof.gy > -deadzone_raw && dof.gy < deadzone_raw)
    dof.gy = 0;
  if(dof.gz > -deadzone_raw && dof.gz < deadzone_raw)
    dof.gz = 0;
#endif
}
void LSM9DS1::calcAccel() {
  ...
  /*ax = dof.convAccel(dof.ax);
  ay = dof.convAccel(dof.ay);
  az = dof.convAccel(dof.az);*/
  // Apply calibration
  dof.ax = (dof.ax + dof.ax_dcbias)*dof.ax_scalebias;
  dof.ay = (dof.ay + dof.ay_dcbias)*dof.ay_scalebias;
  dof.az = (dof.az + dof.az_dcbias)*dof.az_scalebias;
#endif
}
void LSM9DS1::calcMag() {
  ...
  mz = dof.convMag(mz);*/
  // Apply calibration
  dof.mx = (dof.mx + dof.mx_dcbias)*dof.mx_scalebias;
  dof.my = (dof.my + dof.my_dcbias)*dof.my_scalebias;
  dof.mz = (dof.mz + dof.mz_dcbias)*dof.mz_scalebias;
#endif
}
...
void LSM9DS1::calcPitch() {
  pitch = atan2( dof.ay , -dof.az );
}
void LSM9DS1::calcRoll() {
  pitch = calcPitch(dof.ax, dof.ay, dof.az);
  roll = atan2( dof.ax , -dof.ay*sin(pitch) + dof.az*cos(pitch) );
}
void LSM9DS1::calcYaw() {
  pitch = calcPitch(dof.ax, dof.ay, dof.az);
  roll = calcRoll(dof.ax, dof.ay, dof.az);

  float bfx = (dof.mx)*cos(roll) + (dof.my)*sin(pitch)*sin(roll) - (dof.mz)*cos(pitch)*sin(roll);
  float bfy = (dof.my)*cos(pitch) + (dof.mz)*sin(pitch);
  yaw = atan2( bfx , -bfy );
} 
