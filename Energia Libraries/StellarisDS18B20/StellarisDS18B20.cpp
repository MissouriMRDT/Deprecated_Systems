...
float DS18B20::GetData(void)
{
    uint16_t temp;
    reset();
    write_byte(DS1820_SKIP_ROM);  // skip ROM command					0xCC
    write_byte(DS1820_CONVERT_T); // convert T command					0x44
    /////////////////////////////////OW_HI
    delayMicroseconds(750);
    reset();														//	reset
    write_byte(DS1820_SKIP_ROM);		// skip ROM command				0xCC
    write_byte(DS1820_READ_SCRATCHPAD); // read scratchpad command		0xBE
    temp = ReadDS1820();
	
	int16_t stemp = (int16_t)temp;
    return((float)stemp*625/10000);
}
