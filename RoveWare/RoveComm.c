...
int32_t roveComm_InitUdpCFG(rovecomm_socket* rove_socket, uint8_t* local_ip_address, int32_t local_port) {
    ...
    if (rove_socket->socket_fd == COMMS_SOCKET_ERROR) {
        return roveComm_CatchNdkERRNO(rove_socket->ndk_fd_error = fdError() );
    }//endif
    ...
    memset(&(rove_socket->local_ip.address), COMMS_NULL_int32_t , rove_socket->local_ip.address_byte_cnt );
...
int32_t roveComm_GetUdpMSG(rovecomm_socket* rove_socket, rovecomm_protocol* rove_data) {
    ...
    {
        return roveComm_CatchRoveERRNO(rove_data->data_error = NO_RECV_BYTES);
    }//end if
    ...
    if (rove_data->recv_byte_cnt)
    {
        ...
        {
            return roveComm_CatchRoveERRNO(rove_data->data_error = NO_DATA_BYTES);
        }//end if
...
//TODO errno PARSE_ERROR
        default:
            ...
            return rove_data->data_error = COMMS_PARSE_UDP_ERRNO;
    }//switch
}//endfnctn
