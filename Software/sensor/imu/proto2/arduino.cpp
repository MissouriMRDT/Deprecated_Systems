...
#define LSM9DS1_M  0B0011100 // Would be 0x1D if SDO_M is HIGH
#define LSM9DS1_AG 0B1101010 // Would be 0x6B if SDO_AG is HIGH
...
{  
//  dof.readMag();
//  Serial.println(dof.convMag(dof.mx+750), 2); //+750
//  Serial.println(dof.convMag(dof.my-2450), 2); //-2450
//  Serial.println(dof.convMag(dof.mz), 2);
...
float calcYaw(float ax, float ay, float az, float mx, float my, float mz) // tilt compensated heading
{
  float pitch = calcPitch(ax,ay,az);
  float roll = calcRoll(ax,ay,az);
//  
