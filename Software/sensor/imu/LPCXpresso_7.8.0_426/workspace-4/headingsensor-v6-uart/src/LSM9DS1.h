...
public:
	...
	float gxc, gyc, gzc; // calculated x, y, and z axis readings of the gyroscope
	float axc, ayc, azc; // calculated x, y, and z axis readings of the accelerometer
	float mxc, myc, mzc; // calculated x, y, and z axis readings of the magnetometer
	...
	uint16_t begin(gyro_scale gScl = G_SCALE_245DPS, accel_scale aScl =
			...
			G_ODR_119_BW_14, accel_odr aODR = A_ODR_50, mag_odr mODR = M_ODR_40,
			mag_xy_om = M_XY_OM_M, mag_z_om = M_Z_OM_M);
	 ...
	 * NOTE: accel values must be accurate to accurately calculate pitch. */
	void calcPitch();

	/* calcRoll() -- Calculates roll based on pitch and accel values.
	 * NOTE: pitch and accel values must be accurate to accurately calculate roll. */
	void calcRoll();

	/* calcYaw() -- Calculates yaw based on pitch, roll, and mag values (tilt-compensated heading).
	 * NOTE: pitch, roll, accel, and mag values must be accurate to accurately calculate yaw. */
	void calcYaw();
