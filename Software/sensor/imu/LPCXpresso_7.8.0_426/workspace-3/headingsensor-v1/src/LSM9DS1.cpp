...
LSM9DS1::LSM9DS1(interface_mode interface, unsigned char agAddr, unsigned char mAddr) {
	// interfaceMode will keep track of whether we're using SPI or I2C:
	interfaceMode = interface;

	// mAddress and agAddress will store the 7-bit I2C address, if using I2C.
	// If we're using SPI, these variables store the chip-select pins.
	mAddress = mAddr;
	agAddress = agAddr;
}

unsigned short LSM9DS1::begin(gyro_scale gScl, accel_scale aScl, mag_scale mScl, gyro_odr gODR,
...
/*void LSM9DS1::agCal(float * gbias, float * abias) {
 ...
 unsigned char c = agReadByte(CTRL_REG9);
...
float LSM9DS1::calcGyro(signed short gyro) {
	// Return the gyro raw reading times our pre-calculated DPS / (ADC tick):
	return gRes * gyro;
}
float LSM9DS1::calcAccel(signed short accel) {
	// Return the accel raw reading times our pre-calculated g's / (ADC tick):
	return aRes * accel;
}
float LSM9DS1::calcMag(signed short mag) {
...
void LSM9DS1::configGyroInt(unsigned char int1Cfg, unsigned short int1ThsX, unsigned short int1ThsY,
		unsigned short int1ThsZ, unsigned char duration) {
...
//    SPIwriteByte(agAddress, subAddress, data);
		;
	}
}
...
//    return SPIreadByte(agAddress, subAddress);
		;
	}
}
void LSM9DS1::agReadBytes(unsigned char subAddress, unsigned char * dest, unsigned char count) {
// Whether we're using I2C or SPI, read multiple bytes using the
// gyro-specific I2C address or SPI CS pin.
	if (interfaceMode == MODE_I2C) {
//    I2CreadBytes(agAddress, subAddress, dest, count);
	} else if (interfaceMode == MODE_SPI) {
//    SPIreadBytes(agAddress, subAddress, dest, count);
	}
}
...
void LSM9DS1::mReadBytes(unsigned char subAddress, unsigned char * dest, unsigned char count) {
// Whether we're using I2C or SPI, read multiple bytes using the
// accelerometer-specific I2C address or SPI CS pin.
	if (interfaceMode == MODE_I2C) {
...
/* Data Transmission (SPI/I2C)
 ...
 //  pinMode(agAddress, OUTPUT);
 //  digitalWrite(agAddress, HIGH);
 ...
 void LSM9DS1::SPIwriteByte(unsigned char csPin, unsigned char subAddress, unsigned char data) {
 ...
 unsigned char LSM9DS1::SPIreadByte(unsigned char csPin, unsigned char subAddress) {
 //  unsigned char temp;
 ...
 void LSM9DS1::SPIreadBytes(unsigned char csPin, unsigned char subAddress, unsigned char * dest, unsigned char count) {
 //  digitalWrite(csPin, LOW); // Initiate communication
